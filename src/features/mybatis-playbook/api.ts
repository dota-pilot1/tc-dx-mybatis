...
export function createArchitectureDocument(topicId: string, body: { title: string; content: string }) { return request<ArchitecturePlaybookCategory | undefined>(`/mybatis-playbook/topics/${topicId}/documents`, { method: "POST", body, errorMessage: "MyBatis 문서를 만들지 못했습니다." }); }
export function updateArchitectureDocument(id: string, body: { title?: string; content?: string }) { return request<ArchitecturePlaybookCategory | undefined>(`/mybatis-playbook/documents/${id}`, { method: "PATCH", body, errorMessage: "MyBatis 문서를 저장하지 못했습니다." }); }
