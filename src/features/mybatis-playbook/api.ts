...
export function reorderArchitectureCategories(categoryIds: string[]) { return request<ArchitecturePlaybookCategory[]>("/mybatis-playbook/categories/reorder", { method: "POST", body: { categoryIds }, errorMessage: "MyBatis 영역 순서를 저장하지 못했습니다." }); }
...
export function reorderArchitectureTopics(categoryId: string, topicIds: string[]) { return request<ArchitecturePlaybookCategory | undefined>(`/mybatis-playbook/categories/${categoryId}/topics/reorder`, { method: "POST", body: { topicIds }, errorMessage: "MyBatis 주제 순서를 저장하지 못했습니다." }); }
...
export function aiEditArchitectureDocument(id: string, body: { content: string; instruction: string }) { return request<{ content: string }>(`/mybatis-playbook/documents/${id}/ai-edit`, { method: "POST", body, timeoutMs: 180_000, errorMessage: "AI 편집 요청 시간이 초과되었습니다. 문서 분량을 줄이거나 잠시 후 다시 시도해 주세요." }); }
...
export function reorderArchitectureDocuments(topicId: string, documentIds: string[], parentId?: string | null) { return request<ArchitecturePlaybookCategory | undefined>(`/mybatis-playbook/topics/${topicId}/documents/reorder`, { method: "POST", body: { documentIds, parentId: parentId ?? null }, errorMessage: "MyBatis 문서 순서를 저장하지 못했습니다." }); }
