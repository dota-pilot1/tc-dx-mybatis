...
function TitleDialog({ state, busy, onClose, onSave }: { state: TitleDialog; busy: boolean; onClose: () => void; onSave: () => void }) { return <DialogFrame title={`${state.kind === "category" ? "Debugging 영역" : "Debugging 주제"} 삭제`} tone="danger" size="sm" onClose={onClose}><p className="text-sm font-semibold text-text-secondary">이 항목을 삭제할까요?</p><Actions busy={busy} deleting onClose={onClose} onSave={onSave} /></DialogFrame>; }
