...
type TitleDialog = { kind: "category" | "topic"; mode: "delete"; target: ArchitecturePlaybookCategory | ArchitecturePlaybookTopic };
...
function ArchitecturePlaybookModule() {
  ...
  function openDeleteTitleDialog(target: ArchitecturePlaybookCategory | ArchitecturePlaybookTopic, kind: "category" | "topic") { setTitleDialog({ kind, mode: "delete", target }); }
  ...
  async function saveInlineTitle(kind: "category" | "topic", target: ArchitecturePlaybookCategory | ArchitecturePlaybookTopic, nextTitle: string) {
    if (!nextTitle.trim() || busy) return; setBusy(true);
    try {
      if (kind === "category") await updateArchitectureCategory(target.id, nextTitle.trim());
  ...
  async function saveDocumentTitle(target: ArchitecturePlaybookDocument, nextTitle: string) {
    if (!nextTitle.trim() || busy) return; setBusy(true);
    try {
...
function TitleDialog({ state, busy, onClose, onSave }: { state: TitleDialog; busy: boolean; onClose: () => void; onSave: () => void }) { return <DialogFrame title={`${state.kind === "category" ? "Architecture 영역" : "Architecture 주제"} 삭제`} onClose={onClose}><p className="text-sm font-semibold text-text-secondary">이 항목을 삭제할까요?</p><Actions busy={busy} deleting onClose={onClose} onSave={onSave} /></DialogFrame>; }
