import { Check, CircleHelp, CloudCog, FileText, Pencil, Plus, RefreshCw, Trash2, X } from "lucide-react";
...
function CicdPlaybookModule() {
  ...
  return <div className="flex min-w-0 flex-1 flex-col">
    ...
    {deleteTarget && <DialogFrame title={`${deleteTarget.kind === "category" ? "CI/CD 영역" : "CI/CD 주제"} 삭제`} tone="danger" size="sm" onClose={() => setDeleteTarget(null)}><p className="text-sm font-semibold text-text-secondary">이 항목을 삭제할까요?</p><Actions busy={busy} deleting onClose={() => setDeleteTarget(null)} onSave={() => void deleteTitle()} /></DialogFrame>}
