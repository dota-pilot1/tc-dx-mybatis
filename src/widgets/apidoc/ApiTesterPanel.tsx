...
function ResponseCopyButton({
  ...
  return (
    <button
      ...
      title="응답 복사"
      aria-label="응답 복사"
      className="absolute right-3 top-3 z-10 flex h-7 items-center gap-1.5 rounded-md border border-surface-border-soft bg-surface-raised/90 px-2 text-[11px] font-bold text-text-secondary shadow-sm backdrop-blur transition-colors hover:bg-surface-raised hover:text-text-primary disabled:cursor-wait disabled:opacity-60"
    >
      {isCopied ? <Check className="size-3.5 text-brand-primary" /> : <Copy className="size-3.5" />}
...
function JsonResponseBody({
  ...
  for (const match of formatted.matchAll(JSON_TOKEN_PATTERN)) {
    const token = match[0];
    const index = match.index ?? 0;
    if (index > cursor) parts.push(formatted.slice(cursor, index));

    const isKey = formatted.slice(index + token.length).match(/^\s*:/);
  ...
  if (cursor < formatted.length) parts.push(formatted.slice(cursor));

  return (
    <div className="relative overflow-hidden rounded-md border border-surface-border-soft bg-surface-muted shadow-inner">
      <ResponseCopyButton
        isCopied={isCopied}
        isCopying={isCopying}
        onCopy={onCopy}
      />
      <pre className="overflow-x-auto px-4 pb-3 pt-12 font-mono text-[12px] leading-6 whitespace-pre-wrap break-all">
        {parts}
      </pre>
    </div>
  );
...
function ApiTesterPanel({
  ...
  const [isCopyingResponse, setIsCopyingResponse] = useState(false);
  ...
  const resolvedRequestUrl = useMemo(() => {
    try {
      return buildApiRequestUrl(content.url, content.params, envVars);
    } catch {
      return resolvedUrl;
    }
  }, [content.url, content.params, envVars, resolvedUrl]);
  const hasEnvVar = content.url.includes("{{");
  const hasParams = countEnabled(content.params) > 0;
  ...
  const handleSend = async () => {
    ...
    let url: URL;
    try {
      url = new URL(buildApiRequestUrl(content.url, content.params, vars));
    } catch {
      setResponse({
        status: 0,
        statusText: "Invalid URL",
        headers: {},
        body: `유효하지 않은 URL입니다: ${content.url}`,
        durationMs: 0,
  ...
  const handleCopyResponse = async () => {
    if (!response || isCopyingResponse) return;

    setIsCopyingResponse(true);
    try {
      const text = isJsonString(response.body)
            ...
            {isSending ? (
              ...
              <div className="space-y-2">
                {isJsonString(response.body) ? (
                  <JsonResponseBody
                    body={response.body}
                    isCopied={isResponseCopied}
                    isCopying={isCopyingResponse}
                    onCopy={() => void handleCopyResponse()}
                  />
                ) : (
                  <div className="relative overflow-hidden rounded-md border border-surface-border-soft bg-surface-muted shadow-inner">
                    <ResponseCopyButton
                    ...
                    <pre
                      className={
                        "whitespace-pre-wrap break-all px-4 pb-3 pt-12 font-mono text-[12px] leading-5 " +
