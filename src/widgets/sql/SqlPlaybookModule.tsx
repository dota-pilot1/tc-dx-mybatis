...
import {
  createSqlCategory, createSqlDocument, createSqlTopic, deleteSqlCategory, deleteSqlDocument, deleteSqlTopic,
  listSqlPlaybook, moveSqlDocument, updateSqlCategory, updateSqlDocument, updateSqlTopic,
  type SqlPlaybookCategory, type SqlPlaybookDocument, type SqlPlaybookTopic,
} from "../../features/sql-playbook/api";

type TitleDialog = { kind: "category" | "topic"; mode: "delete"; target: SqlPlaybookCategory | SqlPlaybookTopic };
type DocumentDialog = { mode: "create" | "edit" | "delete"; target?: SqlPlaybookDocument };
...
function SqlPlaybookModule() {
  const [categories, setCategories] = useState<SqlPlaybookCategory[]>([]);
  ...
  const [detail, setDetail] = useState<SqlPlaybookDocument | null>(null);
  ...
  async function load(nextCategoryId?: string, nextTopicId?: string, nextDocumentId?: string) {
    try {
      const items = await listSqlPlaybook();
    ...
    } catch (reason: unknown) { setError(reason instanceof Error ? reason.message : "Sql 플레이북을 불러오지 못했습니다."); }
  }
  useEffect(() => { void load(); }, []);

  function openDeleteTitleDialog(target: SqlPlaybookCategory | SqlPlaybookTopic, kind: "category" | "topic") { setTitleDialog({ kind, mode: "delete", target }); }
  ...
  async function createInline(kind: "category" | "topic") {
    ...
    try {
      if (kind === "category") { const next = await createSqlCategory(nextTitle); setInlineTitle((current) => ({ ...current, category: "" })); await load(next?.id ?? category?.id); }
      else if (category) { const next = await createSqlTopic(category.id, nextTitle); const created = next?.topics.find((item) => item.title === nextTitle); setInlineTitle((current) => ({ ...current, topic: "" })); await load(category.id, created?.id ?? topic?.id); }
    } catch (reason: unknown) { setError(reason instanceof Error ? reason.message : "추가하지 못했습니다."); } finally { setBusy(false); }
  }
  async function saveInlineTitle(kind: "category" | "topic", target: SqlPlaybookCategory | SqlPlaybookTopic, nextTitle: string) {
    if (!nextTitle.trim() || busy) return; setBusy(true);
    try {
      if (kind === "category") await updateSqlCategory(target.id, nextTitle.trim());
      else await updateSqlTopic(target.id, nextTitle.trim());
  ...
  async function saveDocumentTitle(target: SqlPlaybookDocument, nextTitle: string) {
    if (!nextTitle.trim() || busy) return; setBusy(true);
    try {
      await updateSqlDocument(target.id, { title: nextTitle.trim() });
  ...
  async function deleteTitleDialog() {
    ...
    try { if (titleDialog.kind === "category") await deleteSqlCategory(titleDialog.target.id); else await deleteSqlTopic(titleDialog.target.id); await load(category?.id); setTitleDialog(null); }
  ...
  async function saveDocumentDialog() {
    ...
    try { if (documentDialog.mode === "edit" && documentDialog.target) await updateSqlDocument(documentDialog.target.id, { title: title.trim(), content: body }); else await createSqlDocument(topic.id, { title: title.trim(), content: body }); await load(category?.id, topic.id, documentDialog.target?.id); setDocumentDialog(null); }
...
function TitleDialog({ state, busy, onClose, onSave }: { state: TitleDialog; busy: boolean; onClose: () => void; onSave: () => void }) { return <DialogFrame title={`${state.kind === "category" ? "Sql 영역" : "Sql 주제"} 삭제`} onClose={onClose}><p className="text-sm font-semibold text-text-secondary">이 항목을 삭제할까요?</p><Actions busy={busy} deleting onClose={onClose} onSave={onSave} /></DialogFrame>; }
...
function DetailDialog({ document, onClose, onEdit }: { document: SqlPlaybookDocument; onClose: () => void; onEdit: () => void }) { return <div className="fixed inset-0 z-40 grid place-items-center bg-[color-mix(in_srgb,var(--background)_72%,transparent)] p-4"><div className="flex max-h-[90vh] w-full max-w-4xl flex-col rounded-xl border border-surface-border bg-surface-raised p-5 shadow-2xl"><div className="flex items-center justify-between"><h2 className="truncate text-lg font-black text-text-primary">{document.title}</h2><button type="button" onClick={onClose} className="ui-icon-button h-8 w-8 rounded-full"><X className="size-4" /></button></div><div className="mt-4 min-h-0 overflow-y-auto rounded-md border border-surface-border-soft p-3"><LexicalEditor key={document.id} initialState={document.content} onChange={() => undefined} readOnly minHeight="480px" /></div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-md px-3 py-2 text-xs font-black text-text-secondary">닫기</button><button type="button" onClick={onEdit} className="inline-flex items-center gap-1.5 rounded-md bg-brand-primary px-3 py-2 text-xs font-black text-text-on-brand"><Pencil className="size-3.5" />수정</button></div></div></div>; }

export default SqlPlaybookModule;
